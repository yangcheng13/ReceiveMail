...
public class ExcelUtil {
    ...
    static {
        ...
        validateMap.put(String.class, new CellType[] {CellType.STRING});
        validateMap.put(Double.class, new CellType[] {CellType.NUMERIC});
        validateMap.put(Date.class,
                new CellType[] {CellType.NUMERIC, CellType.STRING});
     ...
     */
    public static <T> void exportExcel(Map<String, String> headers,
    ...
    private static <T> void write2Sheet(HSSFSheet sheet,
            Map<String, String> headers, Collection<T> dataset,
            String pattern) {
        // 时间格式默认"yyyy-MM-dd"
        ...
        while (it1.hasNext()) {
            ...
            if (headers.containsKey(key)) {
                ...
                HSSFRichTextString text =
                        new HSSFRichTextString(headers.get(key));
            ...
            try {
                if (t instanceof Map) {
                    ...
                    while (it2.hasNext()) {
                        ...
                        cellNum = setCellValue(cell, value, pattern, cellNum,
                                null, row);

                        cellNum++;
                    }
                } else {
                    List<FieldForSortting> fields =
                            sortFieldByAnno(t.getClass());
                    int cellNum = 0;
    ...
    private static int setCellValue(HSSFCell cell, Object value, String pattern,
            int cellNum, Field field, HSSFRow row) {
                    ...
                    for (FieldForSortting ffs : fields) {
                        ...
                        if (field.getType().isArray()) {
                            ...
                            for (int i = 0; i < count; i++) {
                                ...
                                String errMsg =
                                        validateCell(cell, field, cellIndex);
                            ...
                            if (StringUtils.isBlank(errMsg)) {
                                ...
                                if (field.getType().equals(Date.class) && cell
    ...
    private static String validateCell(Cell cell, Field field, int cellNum) {
        ...
        if (cellTypeArr == null) {
            result = MessageFormat.format("Unsupported type [{0}]",
                ...
                if (cell.getCellTypeEnum() == CellType.NUMERIC) {
                    ...
                    if (!Double.isNaN(annoCell.valid().lt())) {
                        if (!(cellValue < annoCell.valid().lt())) {
                            result = MessageFormat.format(
                                    "the cell [{0}] value must less than [{1}]",
                    ...
                    if (!Double.isNaN(annoCell.valid().gt())) {
                        if (!(cellValue > annoCell.valid().gt())) {
                            result = MessageFormat.format(
                                    "the cell [{0}] value must greater than [{1}]",
                    ...
                    if (!Double.isNaN(annoCell.valid().le())) {
                        if (!(cellValue <= annoCell.valid().le())) {
                            result = MessageFormat.format(
                                    "the cell [{0}] value must less than or equal [{1}]",
                    ...
                    if (!Double.isNaN(annoCell.valid().ge())) {
                        if (!(cellValue >= annoCell.valid().ge())) {
                            result = MessageFormat.format(
                                    "the cell [{0}] value must greater than or equal [{1}]",
